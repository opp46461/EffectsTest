...
namespace LiquidVolumeFX
{
    ...
    public class LiquidVolume : MonoBehaviour
    {
        ...
        [SerializeField]
        bool _fixedSpillPoint = false;

        public bool fixedSpillPoint
        {
            get { return _fixedSpillPoint; }
            set
            {
                if (_fixedSpillPoint != value)
                {
                    _fixedSpillPoint = value;
                }
            }
        }
        ...
        void RefreshMaterialProperties()
        {
            ...
            if (_level == 0)
                ...
                liquidActivate = 1;
            liqMat.SetFloat("_LiquidRaySteps", _liquidRaySteps * liquidActivate);
            liqMat.SetFloat("_SmokeRaySteps", _smokeRaySteps * liquidActivate);
            ...
            liqMat.SetFloat("_FoamRaySteps", _foamRaySteps * liquidActivate);
        ...
        /// <summary>
        /// Computes approximate point where liquid starts pouring over the flask when it's rotated计算旋转时液体开始溢出烧瓶的大约点
        /// </summary>
        /// <returns><c>true</c>, if spill point is detected, <c>false</c> otherwise.</returns>
        /// <param name="spillPosition">Returned spill position in world space coordinates.</param>
        ...
        void UpdateSpillPointGizmo()
        {
            ...
            Vector3 spillPoint = Vector3.zero;
            float spillAmount = 0;
            if (GetSpillPoint(out spillPoint, out spillAmount))
            {
                spillPointGizmo.transform.position = spillPoint;
                spillPointGizmo.SetActive(true);
                level -= spillAmount / 10f + 0.001f;
                if (level < limitLevel && limitLevel != 0)
                {
                    level = limitLevel;
                }
            }
            ...
            if (spillAmount!=0)
            {
                if (WaterEffect == null && transform.childCount != 0)
                {
                ...
                {
                    if (!fixedSpillPoint)
                        WaterEffect.transform.position = spillPointGizmo.transform.position;
                    WaterEffect.Play();
                }
            }
        }
